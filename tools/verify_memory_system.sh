...
if crontab -l 2>/dev/null | grep -q "memory_consolidate.py"; then
    echo -e "${GREEN}✅ Configured${NC}"
    CRON_LINE=$(crontab -l | grep memory_consolidate.py | head -1)
...
echo ""
echo "6. Tool files:"
TOOLS=("proactive_memory.py" "memory_search_all.py" "memory_consolidate.py" "memory_autocommit.py" "memory_apply_consolidation.py")
