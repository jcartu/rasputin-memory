...
async def embed(req: EmbedRequestFlex):
    ...
    return {"embeddings": embeddings.tolist(), "model": "nomic-embed-text-v1.5", "device": str(model.device)}
...
async def openai_embed(req: EmbedRequest):
    ...
    return {
        "object": "list",
        "data": [{"object": "embedding", "embedding": e.tolist(), "index": i} for i, e in enumerate(embeddings)],
