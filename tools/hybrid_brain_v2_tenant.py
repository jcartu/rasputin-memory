#!/usr/bin/env python3
"""
Hybrid Brain — Multi-tenant memory API server.

Drop-in replacement for hybrid_brain.py with per-agent tenant isolation.
Each agent operates in an isolated memory namespace using tenant-scoped
Qdrant filters. Same hybrid search pipeline (vector + graph + BM25 + reranker).

Usage:
    python3 hybrid_brain_v2_tenant.py                    # Start server on port 7777
    python3 hybrid_brain_v2_tenant.py --port 7778        # Custom port
    python3 hybrid_brain_v2_tenant.py --test             # Run test queries
"""
...
GRAPH_NAME = os.environ.get("FALKORDB_GRAPH", "brain")
...
def get_embedding(text, prefix="search_query: "):
    ...
    nomic-embed-text v2 uses task prefixes: 'search_query: ' for queries, 'search_document: ' for docs.
...
def extract_entities_fast(text):
    ...
    KNOWN_PROJECTS = {
        "llm-proxy", "hybrid_brain", "second brain", "Manus",
    }
...
def _build_entity_lookup():
    ...
    _projects = {
        ...
        "llm-proxy": ["llm-proxy", "agent proxy"],
...
def extract_entities(query):
    ...
    # Phase 1: Match known entities (longest match first to handle "user agent" before "user")
